...
def test_definition(model, ddl, stub_mogdb_dialect):
    model_ddl = table_to_ddl(model.__table__, stub_mogdb_dialect)
    assert utils.clean(model_ddl) == utils.clean(ddl)


@pytest.mark.parametrize("model, ddl", models_and_ddls)
def test_reflection(mogdb_session, model, ddl):
    _dialect = mogdb_session.bind.dialect
    metadata = MetaData(bind=mogdb_session.bind)
...
def test_no_table_reflection(mogdb_session):
    metadata = MetaData(bind=mogdb_session.bind)
    with pytest.raises(NoSuchTableError):
        Table('foobar', metadata, autoload=True)


def test_no_search_path_leak(mogdb_session):
    metadata = MetaData(bind=mogdb_session.bind)
    Table('basic', metadata, autoload=True)
    result = mogdb_session.execute("SHOW search_path")
