...
r"""
.. dialect:: mogdb+psycopg2
    :name: psycopg2
    :dbapi: psycopg2
    :connectstring: mogdb+psycopg2://user:password@host:port/dbname[?key=value&key=value...]
...
* ``isolation_level``: This option, available for all mogdb dialects,
...
.. tip::
    ...
    that they are passed as explicit keyword arguments to :func:`_sa.create_engine()`::
        ...
        engine = create_engine(
            "mogdb+psycopg2://scott:tiger@localhost/test",
...
connection URI::
    ...
    engine = sa.create_engine(
        "mogdb+psycopg2://scott:tiger@192.168.0.199:5432/test?sslmode=require"
    )
...
which specifies Unix-domain communication rather than TCP/IP communication::
    ...
    create_engine("mogdb+psycopg2://user:password@/dbname")
...
using ``host`` as an additional keyword argument::
    ...
    create_engine("mogdb+psycopg2://user:password@/dbname?host=/var/lib/mogdb")
...
information provided to make the connections::
    ...
    create_engine(
        "mogdb+psycopg2://user:password@/dbname?host=HostA:port1&host=HostB&host=HostC"
    )
...
The psycopg2 DBAPI can connect to MogDB by passing an empty DSN to the
libpq client library, which by default indicates to connect to a localhost
MogDB database that is open for "trust" connections.  This behavior can be
...
initial scheme::
    ...
    engine = create_engine('mogdb+psycopg2://')
...
which may be passed to :func:`_sa.create_engine`::
    ...
    engine = create_engine(
        "mogdb+psycopg2://scott:tiger@host/dbname",
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  ...
  ``libpq`` PostgreSQL client library::
    ...
    engine = create_engine("mogdb+psycopg2://user:pass@host/dbname?client_encoding=utf8")
  ...
  ``libpq``::
    ...
    engine = create_engine(
        "mogdb+psycopg2://user:pass@host/dbname",
...
follows::
    ...
    engine = create_engine("mogdb+psycopg2://scott:tiger@localhost/test",
